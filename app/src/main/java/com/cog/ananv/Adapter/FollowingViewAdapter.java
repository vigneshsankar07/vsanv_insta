...
public class FollowingViewAdapter extends RecyclerView.Adapter<FollowingViewAdapter.Customizeview> {
    ...
    public FollowingViewAdapter(Context context, List<Feedlist> Feedlist, CustomItemClickListener listener) {
    ...
    public Customizeview onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.followinglistview_adapter, parent, false);
...
//        fontChanger.replaceFonts((ViewGroup) d.findViewById(android.R.id.content));
                    ...
                    if (RequestData != null) {
                        for (int i = 0; i < RequestData.size(); i++) {
                            ...
                            if (strstatu.matches("Success")) {
                                ...
                                followingstatus.setText("FollowingViewAdapter");
    ...
    private void unfollow(final String followeruserid, final String user_id) {
        ...
        builder.setMessage("Are you sure to Unfollow this follower ?")
                .setCancelable(false)
    ...
    private void unfollow_user(String postid, String user_id) {
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .build();
        RetrofitArrayAPI service = retrofit.create(RetrofitArrayAPI.class);
        Call<List<com.cog.ananv.Model.Unfollow>> call = service.unfollow(postid, user_id);
                ...
                try {
                    ...
                    if (RequestData != null) {
                        for (int i = 0; i < RequestData.size(); i++) {
                            String strstatu = RequestData.get(i).getStatus();
                            if (strstatu.matches("Success")) {
                                Intent save = new Intent(activity, HomeFragment.class);
...
//                                        .replace(R.id.host_contentContainer, fragment, fragment.getClass().getSimpleName()).addToBackStack(null).commit();
                                Toast.makeText(activity, " User has been Unfollowed..", Toast.LENGTH_SHORT).show();
//SendSuccess
                            } else {
//SendFailed
                            }
